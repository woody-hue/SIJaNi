...
// Variabel global untuk data jadwal
let scheduleData = [];
...
function loadScheduleData() {
    // Simulasi data - dalam implementasi nyata, ini bisa dari API atau localStorage
    scheduleData = JSON.parse(localStorage.getItem('weddingSchedules')) || [
        {
            id: 1,
            groomName: 'John Doe',
            brideName: 'Jane Smith',
            groomPhone: '08123456789',
            bridePhone: '08987654321',
            date: '2023-11-15',
            time: '10:00',
            location: 'KUA',
            locationDetail: '',
...
function editSchedule(id) {
    ...
    if (schedule) {
        openModal('Edit Jadwal Nikah', schedule);
    }
}
...
function downloadAsCsv() {
    const rows = [];
    const headers = [
        'Tanggal', 'Waktu', 'Pria', 'Wanita', 'HP', 
        'Lokasi', 'Status Berkas', 'Keterangan'
    ];
...
// Fungsi untuk mengecek status login (dari script sebelumnya)
...
// Fungsi untuk registrasi Service Worker (dari script sebelumnya)
...
// Fungsi notifikasi (dari script sebelumnya)
